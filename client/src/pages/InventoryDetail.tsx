...
import { ChevronLeft, ShoppingCart, Heart, AlertCircle, Ruler, QrCode, Shirt, Package, Check } from "lucide-react";
...
export default function InventoryDetail() {
  ...
  const handleAddToCart = () => {
    if (!user) { toast.error("Please sign in to add items to cart"); window.location.href = getLoginUrl(); return; }
        ...
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="space-y-3">
            <div className="relative bg-[#EDE9E3] overflow-hidden aspect-square">
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                {discount > 0 && <span className="bg-red-500 text-white text-[9px] font-bold px-2 py-0.5 uppercase tracking-wide">Sale {discount}% Off</span>}
                {availableCount === 1 && <span className="bg-orange-500 text-white text-[9px] font-bold px-2 py-0.5 uppercase tracking-wide">Last Pair</span>}
                {availableCount > 1 && availableCount <= 3 && <span className="bg-amber-400 text-amber-900 text-[9px] font-bold px-2 py-0.5 uppercase tracking-wide">Low Stock</span>}
              </div>
            </div>
            ...
            <div className="bg-[#050f12] px-5 py-4 flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-[11px] text-white/40 leading-relaxed">Scan this QR in-store to view live sizes, request a fitting room, or complete your purchase.</div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-baseline gap-3 mb-2">
              <span className="text-4xl font-black text-[#0d2430]">
                ₱{(selectedProduct.sellingPrice / 100).toLocaleString()}
              </span>
              {discount > 0 && (
                <span className="text-xl text-gray-300 line-through">
                  ₱{(selectedProduct.srp / 100).toLocaleString()}
                </span>
              )}
            </div>
            {discount > 0 && (
              <div className="flex items-center gap-2 mb-5">
                <span className="bg-red-50 text-red-600 text-xs font-bold px-2 py-0.5 border border-red-100">{discount}% OFF</span>
                <span className="text-xs text-green-600 font-semibold">Save ₱{(savings / 100).toLocaleString()}</span>
              </div>
            )}
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <SizeGuideModal trigger={
                  <button className="flex items-center gap-1 text-[10px] font-semibold tracking-wide text-[#C9A84C] hover:opacity-75 transition-opacity uppercase">
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {allSizes.map(({ size, itemCode: sic, status }) => {
                  ...
                  const sameSkuProducts = inventoryProducts?.filter(p => p.sku === product.sku) || [];
                  const count = sameSkuProducts.filter(p => p.size === size && p.status === "AVAILABLE").length;
                  return (
                    <button key={sic} disabled={!avail} onClick={() => setSelectedSize(size)}
                      ...
                      {size}
                      {avail && count <= 1 && !sel && <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 bg-amber-400 rounded-full" />}
                    </button>
                  );
                })}
              </div>
            ...
            {/* Qty */}
            <div className="mb-6">
              <span className="text-xs font-bold tracking-[.08em] uppercase text-[#0d2430] block mb-3">Quantity</span>
              <div className="flex items-center gap-3">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-9 h-9 border border-gray-200 flex items-center justify-center text-lg hover:bg-gray-50 font-medium">−</button>
                <span className="w-8 text-center font-bold text-[#0d2430]">{quantity}</span>
