...
export default function LastPairSection() {
  ...
  const items = useMemo(() => {
    ...
    data.forEach(p => skuCount.set(p.sku, (skuCount.get(p.sku)||0)+1));
    return data.filter(p => (skuCount.get(p.sku)||0)===1).slice(0,12);
  }, [data]);
  ...
  if (!items.length) return <p className="text-sm text-gray-400 py-4">No last pairs</p>;
