...
export default function ClearanceSection() {
  ...
  const items = useMemo(() => {
    if (!data) return [];
    return data.filter(p=>p.discount>=30).sort((a,b)=>b.discount-a.discount).slice(0,12);
  }, [data]);
  ...
  if (!items.length) return <p className="text-sm text-gray-400 py-4">No clearance items</p>;
